...
def main():
    ...
    if Config.result_out_dir is not None:
        conf_mats = np.zeros((num_replicates, num_classes+1, num_classes+1))
        p_scores = np.zeros((num_replicates, num_classes+1))
        r_scores = np.zeros((num_replicates, num_classes+1))
        f1_scores = np.zeros((num_replicates, num_classes+1))
    ...
    for rep in range(num_replicates):
        ...
        if Config.result_out_dir is not None:
            ...
            #print(rep)
            #print(confusion_matrix(y_true_all, y_pred_all).shape)
            ...
            oa_arr[rep] = np.sum(conf_mats[rep]*np.eye(num_classes + 1, num_classes + 1)) / np.sum(conf_mats[rep])
